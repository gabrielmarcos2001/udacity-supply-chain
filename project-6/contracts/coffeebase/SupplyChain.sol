...
contract SupplyChain is Ownable, FarmerRole, RetailerRole, DistributorRole, ConsumerRole {
  ...
  modifier checkValue(uint _upc) {
    ...
    address payable payableDistributorID = address(uint160(items[_upc].distributorID));
  ...
  function buyItem(uint _upc) public payable 
    ...
    address payable payableOriginFramerID = address(uint160(items[_upc].originFarmerID));
    payableOriginFramerID.transfer(items[_upc].productPrice);
  ...
  function shipItem(uint _upc) sold(_upc) onlyDistributor verifyCaller(items[_upc].ownerID) public 
    {
    // Update the appropriate fields
  ...
  function purchaseItem(uint _upc) onlyConsumer() received(_upc) received(_upc) public 
    // Call modifier to check if upc has passed previous supply chain stage
